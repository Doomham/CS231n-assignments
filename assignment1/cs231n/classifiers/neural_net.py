...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss = np.sum(-np.log(exp_correct / exp_sum))/N + reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      X_batch = np.vstack(X[sample_index, :])
      y_batch = np.hstack(y[sample_index])
      #########################################################################
      #                             END OF YOUR CODE                          #
      #########################################################################
  ...
  def predict(self, X):
    ...
    W2, b2 = self.params['W2'], self.params['b2']
    N, D = X.shape
    score = np.dot(np.maximum(X.dot(W1) + b1, 0), W2) + b2
